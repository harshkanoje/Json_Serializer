...
/* Constants for fixed-size arrays to ensure controlled memory usage [2] */
...
/**
 * Data point structure based on Section 4.1 & 4.2 [3].
 */
typedef struct {
    char timestamp[MAX_TIMESTAMP_LEN];       // "timestamp"
    char meter_datetime[MAX_TIMESTAMP_LEN];  // "meter_datetime"
    double total_m3;                         // "total_m3" (Must be numeric [4])
    char status[MAX_STR_LEN];                // "status"
} DataPoint;

/**
 * Device reading structure based on Section 4.1 [2, 3].
 */
typedef struct {
...
/**
 * Gateway-level metadata based on Section 4.1 [2, 3].
 */
typedef struct {
